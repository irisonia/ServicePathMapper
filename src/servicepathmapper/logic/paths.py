...
from servicepathmapper.common.strings.program_args import ARG_MIN_PATH_LEN
from servicepathmapper.common.strings.runtime_messages import InsufficientResources
...
class _Paths:
    ...
    def map_paths(self) -> paths_types.PathsByServersGroupByLen:
        if not self._validate_resources_for_goal():
            return []
    ...
    def _validate_resources_for_goal(self) -> bool:
        return self._validate_num_servers_to_complete_path()

    def _validate_num_servers_to_complete_path(self) -> bool:
        total_servers = len(self._entities.server_name_to_id)
        if total_servers < self._min_path_len:
            Logger.log(
                InsufficientResources(
                    title=f'Not enough servers for {ARG_MIN_PATH_LEN}.',
                    values={'servers': total_servers, ARG_MIN_PATH_LEN: self._min_path_len},
                    help_topics=[ARG_MIN_PATH_LEN]),
                level=logging.INFO)
            return False
        return True
