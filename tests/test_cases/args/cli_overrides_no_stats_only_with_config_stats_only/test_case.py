...
def test_cli_overrides_no_stats_only_with_config_stats_only(monkeypatch, tmp_config) -> None:
    """Test CLI args override config file values"""

    monkeypatch.setattr('sys.argv', ['prog', '--config', tmp_config, '--config-stats-only'])
    args, _ = get_program_args()

    assert args[program_args.ARG_STATS_ONLY] == 1
    assert args[program_args.ARG_CONFIG_STATS_ONLY] == 1
