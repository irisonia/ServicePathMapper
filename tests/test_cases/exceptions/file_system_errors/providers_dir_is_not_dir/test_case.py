...
def test_exception_providers_dir_is_not_dir() -> None:
    with pytest.raises(FileSystemError) as exc_info:
        main(_get_config(), DummyOutputGenerator())
    actual_clients_dir_path = exc_info.value.values[program_args.ARG_PROVIDERS_DIR]
    assert actual_clients_dir_path.name == 'providers'
    assert exc_info.value.help_topics == [program_args.ARG_PROVIDERS_DIR]
