...
def test_exception_dst_server_not_in_providers_dir() -> None:
    with pytest.raises(FileSystemError) as exc_info:
        main(_get_config(), DummyOutputGenerator())

    dst_server = Path(exc_info.value.values[program_args.ARG_DST_SERVER])
    assert dst_server.name == 'Z'
    assert program_args.ARG_DST_SERVER in exc_info.value.help_topics
    assert program_args.ARG_PROVIDERS_DIR in exc_info.value.help_topics
