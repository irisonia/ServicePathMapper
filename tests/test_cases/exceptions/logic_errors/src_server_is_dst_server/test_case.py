...
def test_exception_src_server_is_dest_server() -> None:
    with pytest.raises(LogicError) as exc_info:
        main(_get_config(), DummyOutputGenerator())

    assert str(exc_info.value.values[program_args.ARG_SRC_SERVER]) == 'b'
    assert str(exc_info.value.values[program_args.ARG_DST_SERVER]) == 'b'
