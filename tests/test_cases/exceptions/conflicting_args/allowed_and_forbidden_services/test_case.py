...
def test_exception_allowed_and_forbidden_services() -> None:
    with pytest.raises(ConflictingArgsError) as exc_info:
        main(_get_config(), DummyOutputGenerator())

    assert program_args.ARG_ALLOWED_SERVICES in exc_info.value.values
    assert program_args.ARG_FORBIDDEN_SERVICES in exc_info.value.values
    assert program_args.ARG_ALLOWED_SERVICES in exc_info.value.help_topics
    assert program_args.ARG_FORBIDDEN_SERVICES in exc_info.value.help_topics
