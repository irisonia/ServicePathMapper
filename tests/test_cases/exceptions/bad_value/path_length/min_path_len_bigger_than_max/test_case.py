...
def test_exception_min_path_len_bigger_than_max() -> None:
    with pytest.raises(BadValueError) as exc_info:
        main(_get_config(), DummyOutputGenerator())

    assert exc_info.value.values == {program_args.ARG_MIN_PATH_LEN: 4, program_args.ARG_MAX_PATH_LEN: 3}
    assert exc_info.value.help_topics == [program_args.ARG_MIN_PATH_LEN, program_args.ARG_MAX_PATH_LEN]
